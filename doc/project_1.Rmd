...
```{r most frequent terms for females}
...
```{r most frequent terms for males}
...
```{r overall frequency of bag of words}
...
```{r plot word frequencies for genders}
...
Note this cell also takes a while to run; followed the same lemmatization process as the TA.
...
##Sentiment Analysis
Now, let's conduct a sentiment analysis on text referring to "love" and see how the sentiments vary between men and women.
...
```{r sentiment analysis dataframe}
sent <- colSums(s_male)
...
```{r plot sentiment analysis by gender}
